...
function StockItemAdd({ handleModalClose, stocks, setStocks, setShowModal }) {
  ...
  const handleSubmit = async () => {
    const token = localStorage.getItem("token");
    const itemId = newStock.item ? newStock.item._id : null;
    const stockData = {
      ...newStock,
      item: itemId,
    };
    ...
    if (response.ok) {
      ...
      if (createdStock.extra.is_new) {
        setStocks((previosStocks) => [...previosStocks, createdStock.data]);
      } else {
        setStocks(
          stocks.map((stock) =>
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 bg-opacity-50 flex items-center justify-center px-2">
      <div className="bg-white p-4 border-2 shadow-md w-full px-3 md:w-6/12">
        <ModalHead heading={"Stock Purchase"} />
        <form
        ...
        >
          ...
          {stockUpdated && (
            <div className="text-sm text-green-500">Stock Updated!</div>
          )}
